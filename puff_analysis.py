...
def heatmap_events(dic, save_location, normalization_method="0 to 1"):
    """
    
    normalization_method can be either "0 to 1" or "z-score"
    """
    control = np.load(easygui.fileopenbox(), allow_pickle=True)
    dic = control
    muscimol = np.load(easygui.fileopenbox(), allow_pickle=True)
    dic = muscimol
    ...
    # Normalize data
    if normalization_method == '0 to 1':         # 0 to 1
    ...
    elif normalization_method == "z-score":      # Z-score
        normalized = np.copy(decimated)
        normalized = stats.zscore(normalized, axis=1)
        normalized, _ = compute_stats.sort_data(normalized, method='StandardScaler',
                                                sorted_first_pc=True, 
                                                binned_01=False)
        
    elif normalization_method == "none":      # no normalization
        normalized = np.copy(decimated)
        normalized, _ = compute_stats.sort_data(normalized, method='StandardScaler',
                                                sorted_first_pc=True, 
                                                binned_01=False)
